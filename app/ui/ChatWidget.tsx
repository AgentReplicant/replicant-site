...
const WD: Record<string, number> = { sunday:0,monday:1,tuesday:2,wednesday:3,thursday:4,friday:5,saturday:6,sun:0,mon:1,tue:2,tues:2,wed:3,thu:4,thur:4,thurs:4,fri:5,sat:6 };

function parseNaturalDay(text: string): Date | null {
  const t = text.trim().toLowerCase();
  const now = new Date();

  const hasTomorrow = /\b(tomorrow|tmrw)\b/i.test(t);
  if (t === "today") return now;
  if (t === "tomorrow" || t === "tmrw") return new Date(now.getTime() + 86400000);

  // Looser match: allow extra words; prefer explicit weekday if present
  const m = t.match(/(?:^|\b)(next\s+)?(sun(?:day)?|mon(?:day)?|tue(?:s|sday)?|wed(?:nesday)?|thu(?:r|rs|rsday)?|fri(?:day)?|sat(?:urday)?)(?:\b|$)/i);
  if (m) {
    const want = WD[m[2].toLowerCase()];
    if (want == null) return null;
    const base = new Date(now);
    let daysAhead = (want - base.getDay() + 7) % 7;
    // “friday” today -> next occurrence; “next friday” -> next week
...
function parseLooseTime(text: string): { mins: number; ap?: "am" | "pm" } | null {
...
export default function ChatWidget() {
  ...
  const [sid] = useState(() => {
    ...
    try {
      ...
      localStorage.setItem(k, rnd); return rnd;
    } catch { return Math.random().toString(36).slice(2); }
  });
  const convoSeed = useMemo(() => Math.random().toString(36).slice(2), [open]);

  /* Open triggers */
  useEffect(() => {
    const fromHash = () => { if (typeof window !== "undefined" && location.hash === "#chat") setOpen(true); };
  ...
  useEffect(() => {
    try {
      ...
      const safeDate: DateFilter = s?.date && typeof s.date.y === "number" && typeof s.date.m === "number" && typeof s.date.d === "number" ? { y: s.date.y, m: s.date.m, d: s.date.d } : null;
      setMessages(s.messages ?? []); setEmail(s.email ?? undefined); setPhone(s.phone ?? undefined);
      setDate(safeDate); setPage(s.page ?? 0); setLastSlots(s.lastSlots ?? null);
      setChosenSlot(s.chosenSlot ?? null); setPending(s.pending ?? null); setMode(s.mode ?? "phone");
    } catch {}
  }, []);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORE_KEY, JSON.stringify({ messages, email, phone, date, page, lastSlots, chosenSlot, pending, mode }));
    } catch {}
  }, [messages, email, phone, date, page, lastSlots, chosenSlot, pending, mode]);

  /* Autoscroll */
  useEffect(() => { const el = wrapRef.current; if (el) el.scrollTop = el.scrollHeight; }, [messages, busy, isTall]);
  ...
  async function logMessage(role: "user" | "assistant", text: string) {
    try {
      await fetch("/api/chatlog", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sessionId: `${sid}`, role, message: text, source: "Replicant site", pageUrl: typeof window !== "undefined" ? window.location.href : "" }) });
    } catch {}
  }
  async function maybeUpsertLeadFromText(text: string) {
    try {
      ...
      await fetch("/api/lead", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: nameMatch, email: emailMatch, phone: phoneDigits || undefined, source: "Replicant site", status: "Engaged" }) });
    } catch {}
  }
  ...
  function appendUser(text: string) {
    ...
    void logMessage("user", text); void maybeUpsertLeadFromText(text);
  }
  ...
  async function callBrain(payload: any) {
    ...
    const body = { ...payload, history: historyRef.current, filters, sessionId: `${sid}:${convoSeed}` };
  ...
  function selectSlotFromUserText(text: string, slots: Slot[]): Slot | null {
    if (/\bmorning|afternoon|evening\b/i.test(text)) return null; // force re-query by window
    const loose = parseLooseTime(text);
    if (!loose) return null;
    let best: Slot | null = null; let bestDelta = Infinity;
  ...
  async function handleBrainResult(data: any) {
    ...
    if (data?.type === "action" && data.action === "open_url" && data.url) {
      appendBot(data.text || "Here’s a secure link:"); appendBot(`👉 ${data.url}`, { link: data.url }); return;
    }
    if (data?.type === "slots" && Array.isArray(data.slots)) {
      setLastSlots(data.slots as Slot[]); setChosenSlot(null); setPending(null);
      appendBot(data.text || "Here are some times in ET. What works?");
      return;
    }
    if (data?.type === "booked") {
      setLastSlots(null); setChosenSlot(null); setPending(null);
      appendBot("All set — invite sent. Talk soon!"); return;
    }
    if (data?.type === "error") {
      appendBot(data.text || "That didn’t go through — want to try another time?"); return;
    }
    if (data?.type === "text" && data.text) { appendBot(data.text); return; }
    if (data?.text) appendBot(data.text);
  }

  async function handleSend(raw?: string) {
    const val = (raw ?? input).trim(); if (!val || busy) return;
    appendUser(val); setInput("");
    ...
    if (lastSlots && !chosenSlot) {
      ...
      // Try to pick from the list (±75m). If user mentions window, we skip and re-query.
      const sel = selectSlotFromUserText(val, lastSlots);
      if (sel) {
        setChosenSlot(sel);
        if (/\b(meet|video)\b/i.test(val)) { setMode("video"); setPending("email"); appendBot("Google Meet works. What’s the best email for the invite?"); }
        else { setMode("phone"); setPending("phone"); appendBot("Phone works. What’s the best number to call?"); }
        return;
      }
      // If they specified a day in this message, pass that to the brain flow
      const maybeDay = parseNaturalDay(val);
      if (maybeDay) {
        const now = new Date(); const chosen = sameYMD(now, maybeDay) ? now : maybeDay;
        const { y, m, d } = ymd(chosen); setDate({ y, m, d }); setPage(0); setBusy(true);
        try { const data = await callBrain({ message: val }); await handleBrainResult(data); }
        finally { setBusy(false); }
        return;
      }
      // Fall through to brain for clarification (window change etc.)
    }

    // Mode follow-up
    if (pending === "mode") {
      if (/\b(meet|video)\b/i.test(val)) { setMode("video"); setPending("email"); appendBot("Great — what’s the best email for the Meet invite?"); }
      else { setMode("phone"); setPending("phone"); appendBot("Phone it is. What’s the best number to call?"); }
      return;
    }
    if (pending === "phone") {
      const digits = onlyDigits(val);
      if (digits.length < 7) { appendBot("Could you share the full number with area code?"); return; }
      setPhone(digits); setPending("email"); appendBot("Thanks. What’s the best email for the calendar invite?");
      return;
    }
    if (pending === "email") {
      if (!EMAIL_RE.test(val)) { appendBot("Mind sharing a valid email for the invite?"); return; }
      setEmail(val);
      if (!chosenSlot) { appendBot("I didn’t catch the time — which time works for you?"); return; }
      setBusy(true);
      try {
        const booked = await callBrain({ pickSlot: { start: chosenSlot.start, end: chosenSlot.end, email: val, mode, phone: mode === "phone" ? phone : undefined } });
        await handleBrainResult(booked);
      } finally { setBusy(false); }
      return;
    }

    // If message contains a day word, lock date so brain can resolve conflicts like "Fri 10am"
    const maybeDay = parseNaturalDay(val);
    if (maybeDay) {
      const now = new Date(); const chosen = sameYMD(now, maybeDay) ? now : maybeDay;
      const { y, m, d } = ymd(chosen); setDate({ y, m, d }); setPage(0); setBusy(true);
    ...
    <>
      ...
      {open && (
        <div
          ...
          style={{ width: isTall ? 600 : 420, maxWidth: "92vw", height: isTall ? "85vh" as any : "620px" }}
        >
          <div className="h-full flex flex-col">
            <div className="bg-white border-b px-4 py-3 flex items-center justify-between shrink-0">
              <div className="font-semibold text-sm text-slate-900">Replicant Assistant</div>
              <div className="flex items-center gap-2">
                <button onClick={() => setIsTall((v) => !v)} className="text-xs text-gray-500 hover:text-black" aria-label="Toggle size">
            ...
            <div className="bg-white border-t p-2 shrink-0">
              <div className="flex gap-2 items-end">
                <textarea
                  ...
                  rows={1}
                />
                <button onClick={() => handleSend(input)} disabled={busy} className="bg-black text-white text-sm px-4 py-2 rounded-xl disabled:opacity-50">Send</button>
              </div>
